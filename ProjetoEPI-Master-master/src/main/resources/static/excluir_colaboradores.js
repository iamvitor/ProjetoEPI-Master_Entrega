// Adicione um ouvinte de eventos aos botões de exclusão
document.querySelectorAll('.excluir').forEach(function(button) {
    button.addEventListener('click', function() {
        if (confirm('Confirma a exclusão?')) {
            const row = this.closest('tr'); // Obtém a linha atual da tabela
            const colaboradorId = this.dataset.colaboradorId; // Acessa o ID corretamente

            // Realize a chamada AJAX para excluir o recurso
            fetch(`/colaboradores/${colaboradorId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
            })
                .then(response => {
                    if (response.ok) {
                        // A exclusão foi bem-sucedida
                        console.log('Colaborador excluído com sucesso.');
                        row.remove(); // Remove a linha da tabela
                    } else {
                        console.error('Não foi possivel. Colaborador esta no uso!');
                        alert('Não foi possivel. Colaborador esta no uso!');
                    }
                })
                .catch(error => {
                    console.error('Erro de rede:', error);
                    alert('Erro de rede: ' + error);
                });
        }
    });
});
